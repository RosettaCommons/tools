...
def create_protocols_interlibrary_dependency_dot_file( g ) :
   ...
   for node in protocols_library_graph.nodes() :
      for node2 in protocols_library_graph.neighbors( node ) :
         if node == node2 : continue # skip loops
...
def create_intralibrary_dependency_graph( g, srcsettings_fname ) :
   libname, topleveldirs = toplevel_directories_from_srcsettings_file( srcsettings_fname )
   topleveldir_graph = pygraph.digraph()
   for topleveldir in topleveldirs :
      topleveldir_graph.add_node( topleveldir )

   liblevel, libcolumn = library_level_and_column_from_src_settingsfile_name( libname, srcsettings_fname )
   prot_levels = library_levels.protocols_levels()
...
def draw_all_protocol_dependencies( g ) :
   colors_for_level = [ "black", "indigo", "crimson", "green", "chocolate", "cyan3", "blue1", "salmon2" ]

   intralibrary_dependency_graphs = {}
   projects = code_utilities.rosetta_projects()
   for lib in projects[ "src" ] :
      if lib.find( "protocols" ) == -1 : continue
      libsrcsettings = lib + ".src.settings"
      level, column = library_level_and_column_from_src_settingsfile_name( "protocols", libsrcsettings )
      libid = str(level) + column
      intralibrary_dependency_graphs[ libid ] = create_intralibrary_dependency_graph( g, libsrcsettings )
   prot_levels = library_levels.protocols_levels()
   protocols_library_graph = create_protocols_interlibrary_dependency_graph( g )
   ...
   for level in xrange(len(prot_levels)) :
      ...
      for column in xrange(len(prot_levels[level])) :
         ...
         line = 'subgraph "cluster_' + sublibname + '" { label="protocols.' + sublibname + '"; color=' + colors_for_level[ int( sublibname[:1] ) - 1 ] + ";"
         ...
         for node in intralib_dep_graph.nodes() :
            ...
            for neighbor in intralib_dep_graph.neighbors( node ) :
               if node == neighbor : continue # skip loops
               line += ' "' + node + '" -> "' + neighbor + '";'
         line += "}" + endl
   ...
   for node in protocols_library_graph.nodes() :
      ...
      for node2 in protocols_library_graph.neighbors( node ) :
         if node == node2 : continue # skip loops
         graph_lines.append( '"' + representative_node[ node ] + '" -> "' + representative_node[ node2 ] + '" [ minlen=10, lhead="cluster_' + node2 + '", ltail="cluster_' + node +'", color=' + nodecolor + ' ];' + endl )
...
if __name__ == "__main__" :
   ...
   if not graphviz_output :
      ...
      open( output_prefix + "_interlibrary_dependencies.dot", "w" ).writelines( graphlines )
      ...
      for lib in projects[ "src" ] :
         ...
         open( output_prefix + "_" + lib + "_intralib_dependencies.dot", "w" ).writelines( graphlines )
