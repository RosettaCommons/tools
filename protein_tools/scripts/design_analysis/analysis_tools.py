...
class AnalysisTools():
    ...
    def verbose(self, list_o):
        ...
        for key in list_o:
            ...
            # with there occurences, there is another method for that
                ...
                self.seq_analysis._verbosely_print_native_dict(self._arg_dict[key])
    ...
    def output_rosetta_dict(self, dictionaries, prefix, models, file="output"):
        name = prefix + file + "rosetta_scores.csv"
    ...
    def output_sequence_logos(self, output, native, sequence="", prefix="", format="", units="", stacks_per_line="", 
        ...
        command = seqlogos.initialize_options(format=format, units=units, stacks_per_line=stacks_per_line, title=title, 
        										...
        										fine_print=fine_print, color_scheme=color_scheme)
        if debug:
...
class RosettaScores():
    ...
    def __init__(self, dict_of_interest, pdb):
        ...
        self.model = pdb
        self.score_table = rs.ScoreTable(pdb)
...
class SequenceLogos():
    ...
    def initialize_options(
        self, format="eps", sequence_type="protein", units="bits", first_index_number="1", lower_bound=None, upper_bound=None, size="large", stacks_per_line="40", title="Sequence Logo", x_axis_label=None, y_axis_height="4.32", y_label="Bits",
        ...
        self.seq_output = ["-o", self.output + "." + format]
        ...
        self.combined_arguments = self.input_file + self.seq_output + self.format + self.sequence_style + self.units + self.first_index + self.size + self.title + self.annotate +\
            ...
            self.fine_print + self.scale_width + self.composition + self.stacks
