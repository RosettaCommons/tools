    ...
    for line in flines :
        ...
        if cr.curr_line == dstor.definition_end_line :
            ...
            if countback == 0 :
                ...
                return
            for linenum in xrange( countback, cr.curr_line ) :
...
if __name__ == "__main__" :
    ...
    if len(sys.argv) > 1 :
        for focused_header in sys.argv[1:] :
            ...
            classes_in_focused_header_deriving_from_refcount = filter( lambda x : class_derives_directly_from_refcount(x), classes_in_focused_header )
            classes_needing_virtccdstors = filter( lambda x : not class_has_virtual_dstor(x), classes_in_focused_header_deriving_from_refcount )
            classes_w_nonvirtdstors = filter( lambda x : class_has_dstor( x ), classes_needing_virtccdstors )
            classes_wo_dstors = filter( lambda x : not class_has_dstor( x ), classes_needing_virtccdstors )
            classes_w_nonvirtdstors_in_header = filter( lambda x : class_has_dstor_in_header(x), classes_w_nonvirtdstors )
            classes_w_nonvirtdstors_in_ccfile = filter( lambda x : not class_has_dstor_in_header(x), classes_w_nonvirtdstors )
    ...
    classes_needing_virtccdstors = filter( lambda x : not class_has_virtual_dstor(x), classes_deriving_from_refcount )
    classes_wo_dstors = filter( lambda x : not class_has_dstor( x ), classes_needing_virtccdstors )
    classes_w_nonvirtdstors = filter( lambda x : class_has_dstor( x ), classes_needing_virtccdstors )
    classes_w_nonvirtdstors_in_header = filter( lambda x : class_has_dstor_in_header(x), classes_w_nonvirtdstors )
    classes_w_nonvirtdstors_in_ccfile = filter( lambda x : not class_has_dstor_in_header(x), classes_w_nonvirtdstors )
