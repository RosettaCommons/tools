...
class Unit:
    ...
    def findSTLTypesInVartype( self, vartype ) :
        container = vartype[ vartype.find( "std::" ): vartype.find("<") ]
    ...
    def makeVarsArStub( self, decl, membvars, style, include_base = True ):
        ...
        for i, v in enumerate(membvars):
            ...
            if v.get('comment'):
                newline.append(  "; " + v['comment'] )
            newline.append( "\n" )
            stub.append( "".join( newline ))
            if v['constptr'] :
    ...
    def insertCCStub(self, decl ):
        ...
        stub.append(self.indent( self.makeFullStub( decl, "save" ), 1 ))
        stub.append("}\n\n")
        ...
        if decl.need_load_construct:
            ...
            stub.append(self.indent( self.makeFullStub( decl, "load_and_construct" ), 1 ))
            stub.append("}\n")
        else:
            stub.append("/// @brief Automatically generated deserialization method\n" )
            stub.append("template< class Archive >\n")
            ...
            stub.append(self.indent( self.makeFullStub( decl, "load" ), 1 ))
            stub.append("}\n")
