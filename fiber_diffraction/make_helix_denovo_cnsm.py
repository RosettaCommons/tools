...
class Helixer(object):
    ...
    """
    def __init__(self, pitch, cn_symm, nsub, vturns, unit, name="helix_denovo", virtuals_name = "virtuals"):
        self.pitch = pitch
	self.cn_symm = cn_symm
        self.nsub = nsub
        self.vturns = vturns
        self.unit = unit
        self.name = name
	...
	#Next layers (up and down)
	E_line+=" + 4*("+root_name+":"+self.vrt_names[i+self.cn_symm]+")"
	E_line+=" + 4*("+root_name+":"+self.vrt_names[i+self.cn_symm+1]+")"
	E_line+=" + 2*("+root_name+":"+self.vrt_names[i+self.cn_symm+2]+")"
    ...
    def __setupVirtualCoordsLines(self):
        ...
        y = array( [0,1,0] )
        o = array( [0,0,0] )
        root =  Virtual(x,y,o,"VRT_0_0_0")
        o_base = array( [0,0,0] )
        root_base =  Virtual(x,y,o_base,"VRT_0_0_0_base")
        #And here will be for loop from 1 to n iterating over negative and positive virtuals
        atomnum = 1
        resnum = 1
	pitch = (-self.nsub/2)*self.pitch/self.cn_symm
	angle_counter = 0 
	#Initial radius can be changed
	r=1
        for i in range(-self.nsub/2, self.nsub/2+1):
            #if i!=0:
		if i % self.cn_symm == 0:
                        ...
                        angle_counter+=1
                zaxis = array([0,0,1])
		angle = -i*(360/self.cn_symm)-angle_counter*(360/self.cn_symm)*float(self.vturns)/float(self.unit)

		############Subunits vrts#############################
#               Rz = self.__rotateMatrix( zaxis, angle )
#               vrt_obj_sub = self.__rotateVrt( root, Rz )
#               vrt_obj_sub.set_name(self.vrt_names_subs[i])
#               #vrt_obj.vrtOrig = root.vrtOrig+[xx,yy,pitch]
#               vrt_obj_sub.vrtOrig = root.vrtOrig+[0,0,pitch]
#               # Normalize
#               vrt_obj_sub.normalize()
#               x = vrt_obj_sub.vrtX
#               y = vrt_obj_sub.vrtY
#               z = cross( x, y )
#               y = cross( z, x )
#               vrt_obj_sub.vrtY = y
#               self.virtuals.extend(vrt_obj_sub.print_pdbline(resnum,atomnum))
#               self.output.append(vrt_obj_sub.display())
		...
		yy = r*sin_theta
                ...
                Rz = self.__rotateMatrix( zaxis, angle )
                vrt_obj = self.__rotateVrt( root_base, Rz )
                vrt_obj.set_name(self.vrt_names[i])
                #vrt_obj.vrtOrig = root.vrtOrig+[xx,yy,pitch]
                vrt_obj.vrtOrig = root_base.vrtOrig+[0,0,pitch]
		# Normalize
                vrt_obj.normalize()
                x = vrt_obj.vrtX
                y = vrt_obj.vrtY
...
#		jump_line2+="  "+self.jump_names[-i]+"_to_com"
	...
	#TODO: Can be done easier for sure
	jump_line1 +="  "+self.jump_names[-1]
...
if '__main__' == __name__:
    ...
    helixer = Helixer(options.pitch, options.cn_symm, options.nsub, options.vturns, options.unit, options.name, options.virtuals_name)
    #Executing pipeline
    helixer.execute()
    helixer.write()
