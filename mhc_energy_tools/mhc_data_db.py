...
# * csv format database -- same as for gen_db -- one question is where to store metadata
...
def setup_parser():
    ...
    * NetMHCII identifies one core per peptide (may be of low confidence), but matrices can identify multiple (or none)
    ...
    source.add_argument('--iedb_existing_mysql', help='name of mysql database holding IEDB-format tables')
    source.add_argument('--iedb_fresh_mysql', help='name of mysql database into which IEDB-format tables will be downloaded and stored, to enable processing (note: blows away exsiting IEDB tables!)')
    # TODO mysql username and password
...
def main(args):
    ...
    elif args.iedb_existing_mysql is not None:
        data = IEDBData.from_mysql(args.iedb_existing_mysql, data_allele_list, args.allele_set)
    elif args.iedb_fresh_mysql is not None:
        IEDBData.download_mysql(args.iedb_fresh_mysql)
        data = IEDBData.from_mysql(args.iedb_fresh_mysql, data_allele_list, args.allele_set)
    name = data.name
...
#        raise Exception('not implemented yet -- need to extract cores')
    ...
    scores = [EpitopeScore(1, [1 if a in hits else None for a in data.std_alleles]) for hits in core_hits.values()]
    epimap = EpitopeMap(9, data.std_alleles, cores, scores)
        
    db = EpitopeDatabase.for_writing(args.db, name, data.std_alleles, 9)
