...
#    $psiblast_command="$PSIBLAST -i $seq -d $DB -j 2 -h 0.001 -b5000 -v5000 -o $blastout";
    $psiblast_command="$PSIBLAST -num_threads 22 -query $seq -db $DB -num_iterations 2 -inclusion_ethresh 0.001 -num_descriptions 5000 -num_alignments 5000 -out $blastout";
    print "Running:\n $psiblast_command\n";
    `$psiblast_command`;
}
...
if(!-e "$blastout.msa")
{
	 $cmd_parseblast = "$parseblast $blastout $blastout.msa -psi";
	 print $cmd_parseblast."\n";
