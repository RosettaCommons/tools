...
class CodeReader :
   ...
   def determine_visibility( self ) :
      ...
      if self.re_pound_if.match( self.commentless_line ) :
         ...
         self.nested_ifdefs.append( (self.if_directive + " " + self.re_splitter.findall( self.commentless_line )[ 1 ], True ) )
      elif self.re_pound_ifdef.match( self.commentless_line ) or self.re_pound_ifndef.match( self.commentless_line ) :
...
class HeavyCodeReader( CodeReader ) :
   def __init__( self ) :
      ...
      self.re_privacy = [ ("private", re.compile( "\s*private\s*:\s*$" )),
      ...
      self.func_patterns = [ ( 'varname', re.compile( '[\w!:+=/$~.]+' ) ), \
                             ( '&', re.compile( r'&' ) ), \
                             ( '(', re.compile( '\(' ) ), \
                             ( ')', re.compile( '\)' ) ), \
                             ( '*', re.compile( '\*' ) ), \
   ...
   def at_class_scope( self ) :
      if len( self.class_stack ) == 0 :
         return False
      if self.class_stack[ -1 ][ 1 ] and self.last_scope_level == self.class_stack[ -1 ][ 2 ] + 1:
         return True
      return False
   ...
   def examine_visible_line( self ) :
      ...
      if self.at_namespace_scope() or self.at_global_scope() :
         ...
         if self.re_namespace_dec.match( self.commentless_line ) :
            ns = self.commentless_line.split("namespace")[ 1 ].strip().split(" ")[ 0 ].strip()
      ...
      if self.re_class_dec.match( self.commentless_line ) :
         ...
         if self.re_class_fwd_dec.match( self.commentless_line ) :
            ...
            if len( self.commentless_line.split(":")) > 1 :
               self.parent_class_name = self.commentless_line.partition(":")[ 2 ].split("{")[ 0 ].strip()
            if class_has_begun:
         ...
         if self.parent_class_name :
            ...
            self.parent_class_name  = parent_str
         if self.scope_level > self.class_stack[ -1 ][ 2 ] : # we've found a "{" on this line
               ...
               while count_paren_depth != 0 :
                  ...
                  if next_rparen_ind == -1 :
                     ...
                     if count_paren_depth == 1 :
                        substr += remaining[ : next_rparen_ind ] # clip the last ")"
                     else :
               ...
               if not self.re_regular_varname.match( self.function_name ) :
                  if ind_of_lparen > 1 :
                     if toks[ ind_of_lparen - 2 ] == "operator" :
                        ...
                        self.function_name = "operator" + toks[ ind_of_lparen - 1 ]
...
def read_classes_from_header( fname, flines, options = None ) :
   ...
   for line in flines :
      ...
      if cr.at_class_scope() :
         if not curr_class or ( curr_class and curr_class.name != cr.current_class_name() ) :
            if curr_class :
               if func :
...
def find_data_members_unassigned_in_assignment_operator( fname, flines, class_data ):
   assigned = find_data_members_assigned_in_assignment_operator( fname, flines, class_data )
   unassigned = []
   if assigned == None :
      return unassigned
   for dm in class_data.data_members :
