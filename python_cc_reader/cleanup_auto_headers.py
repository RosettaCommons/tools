...
def cleanup_autoheaders_for_lines( flines ) :
    ...
    for line in flines :
        ...
        if len(line) > len("// Auto-header: duplicate removed") and line[:len("// Auto-header: duplicate removed")] == "// Auto-header: duplicate removed" :
            continue
        if found_auto_headers :
            ...
            if line == "\n" and (count_line == auto_header_begin + 1) :
                ...
                count_line += 1
                holding_autoheader_declaration = False # two blank lines in a row: do not append an autoheader
                continue
