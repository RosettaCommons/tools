import re
import ..cpp_parser.code_reader as code_reader
...
def group_headers( headers ) :
   # order should be: core, protocols, devel, apps, utility, numeric, ObjexxFCL, STL, everything else
   lib_names = [ "platform", "core", "protocols", "devel", "apps", "utility", "numeric", "ObjexxFCL" ]
...
def add_autoheaders_to_filelines( fname, filelines, headers ) :
   ...
   for line in filelines :
      ...
      if cr.line_is_visible() and pound_define.match( line ) and cr.scope_level == 0 and pound_define_line == 0 :
   ...
   for line in filelines :
      count_line += 1
      if auto_header_start_line != 0 and count_line >= auto_header_start_line and count_line <= last_include_line :
