...
parser.add_option("-s", "--scores_to_keep", dest="scores_to_keep", type='string', 
...
parser.add_option("--no_align", dest="do_align", default="True", action="store_false", #default does align by protein before RMSD
		help="provide this flag to calculate RMSD without doing alignment")
parser.add_option("--conformer_rmsd", dest="do_super", default="False", action="store_true",
		help="provide this flag to calculate ligand RMSD based on conformer differences only by aligning ligands onto each other")
parser.add_option("--centroid", dest="cent_mode", default="False", action="store_true",
		help="calculate distance of ligand centroids between model and native")

(options, args)=parser.parse_args()

print "Conformer RMSD " + str(options.do_super)
print "Centroid mode " + str(options.cent_mode)
	...
	for i in range(len(symmetrical_chains)-1):
		for j in range(i+1, len(symmetrical_chains)):
			...
			if this_rms < best_rms:
				best_rms = this_rms
				best_alignment=(i,j)
	pymol.cmd.align('docked and not hetatm and chain '+symmetrical_chains[best_alignment[0]], 'native and not hetatm and chain '+symmetrical_chains[best_alignment[1]])
...
def calculate_rms(file):
	...
	if options.chains==None:
		...
		for chain in chains.split('+'):
			...
			if options.cent_mode==True:
				docked_cent = centroid.centroid('docked_lig')
				native_cent = centroid.centroid('native_lig')
				dist = math.sqrt((docked_cent[0]-native_cent[0])**2 + (docked_cent[1]-native_cent[1])**2 + (docked_cent[2]-native_cent[2])**2)
				ligand_rmss.append(dist)
			#Align ligand then do RMSD Superimposed rmsd
			if options.do_super==True:
