...
class Token:
    def __init__(self):
        ...
        self.type = (
            "substatement"
        )  # default is a vanilla "I'm part of another statement" type.
        self.depth = 0
        self.index = -1
    ...
    def equivalent(self, other):
        if self.spelling != other.spelling:
            ...
            return False
        elif self.invisible_by_macro != other.invisible_by_macro:
            return False
        elif self.is_visible != other.is_visible:
            return False
        elif self.is_inside_string != other.is_inside_string:
            return False
        return True
...
class Beautifier:
    def __init__(self):
        ...
        self.macros_at_zero_indentation = set(
        ...
        self.pound_if_setting = (
            ""
        )  # can be "take_if" or "take_else" to broadly specify to take one or the other for a file
    ...
    def breakup_line_into_tokens(self, line):
        ...
        while i < len(self.line):
            ...
            if self.line[i] == "/":
                if i + 1 < len(self.line) and (
                    self.line[i + 1] == "/" or self.line[i + 1] == "*"
                ):
                    if start >= 0:
                        self.take_token_for_range(start, i)
                    self.take_token_for_range(i, i + 2)
    ...
    def process_strings_and_comments(self):
        ...
        if self.in_macro_definition:
            ...
            if debug:
                print("still in macro definition", self.line_number)
            if self.this_line_tokens:
                ...
                if self.this_line_tokens[-1].spelling != "\\":
                    self.in_macro_definition = False
                    if debug:
                        print("final line in macro definition", self.line_number)
                return
            else:
                # empty line
                self.in_macro_definition = False
        ...
        while i < len(self.this_line_tokens):
            ...
            if self.raw_string_literal_delim is not None:
                ...
                if tok.spelling == "\\":
                    ...
                    escape_tok = None
            elif len(tok.spelling) == 2 and (
                tok.spelling == "//"
                or (tok.spelling == "/*" and self.macro_definitions_say_line_visible())
            ):
                # comment begin
                tok.is_visible = False
                tok.is_commented = True
        ...
        if process_as_macro_line:
            self.process_macro_line()
    ...
    def process_macro_line(self):
        ...
        if tok0.spelling == "#define":
            ...
            if debug:
                print("#defined:", self.this_line_tokens[1].spelling)
            if self.this_line_tokens[-1].spelling == "\\":
                self.in_macro_definition = True
                if debug:
                    print("in macro definition", self.line_number)
            ...
            if len(self.this_line_tokens) > 1:
                ...
                if name in self.defined_macros:
                    ...
                    self.nested_ifdefs.append(
                        (name, "ifdef", self.nested_ifdefs[-1][2])
                    )
                else:
            ...
            if len(self.this_line_tokens) > 1:
                ...
                if name not in self.defined_macros:
                    self.nested_ifdefs.append(
                        (name, "ifndef", self.nested_ifdefs[-1][2])
                    )
                else:
                    self.nested_ifdefs.append((name, "ifndef", False))
        elif tok0.spelling == "#if":
            name = " ".join(
                [x.spelling for x in self.this_line_tokens[1:] if not x.is_commented]
            )
    ...
    def find_next_visible_token(self, i, stack=None):
        if debug:
            self.print_entry("find_next_visible_token", i, stack)
        while i < len(self.all_tokens):
            if self.all_tokens[i].is_visible:
                return i
            if stack != None:
    ...
    def process_statement(self, i, stack):
        ...
        if i_spelling in self.for_types:
            ...
            i_spelling == "public"
            or i_spelling == "protected"
            or i_spelling == "private"
        ):
    ...
    def handle_read_all_tokens_error(self, function_name, stack):
        print(
            "ERROR: Ran out of tokens in function",
            function_name,
            "in file",
            self.filename,
        )
            ...
            if elem:
                print(
                    ...
                    (
                        ...
                        % (
                            elem.line_number + 1,
                            self.all_lines[elem.line_number].strip(),
                        )
                    ),
                    "vis" if elem.is_visible else "invis",
                )
    ...
    def process_template(self, i, stack):
        if debug:
            self.print_entry("process_template", i, stack)
        ...
        while i < len(self.all_tokens):
            ...
            if not self.all_tokens[i].is_visible:
                ...
                if template_depth == 0:
                    ...
                    if debug:
                        self.print_entry("exitting process_template", i, stack)
                    return i + 1
    ...
    def var_in_class_dec_initializer(self, stack):
        ...
        return (
            len(stack) >= 4
            and stack[-1].type == "statement"
            and stack[-2].type == "function-scope"
            and stack[-3].type == "function"
            and stack[-4].type == "class-scope"
        )
    ...
    def could_be_lambda(self, token_list):
        ...
        for tok_ind in range(len(token_list)):
            ...
            if tok_ind == 0:
                ...
                if tok == "throw":
                    if seen_throw or seen_nothrow:
                        return False
                    seen_throw = True
                elif tok == "nothrow":
                    if seen_nothrow or seen_throw:
                        return False
                    seen_nothrow = True
            # elif tok == "{" :
    ...
    def process_simple_statement(self, i, stack):
        if debug:
            self.print_entry("process_simple_statement", i, stack)
        ...
        bracket_stack = []
        lambda_detection_stack = [
            []
        ]  # what symbols have we found consistent with a lambda function declaration given our current bracket stack?

        if (
            self.all_tokens[i].spelling in open_brackets
            and not self.all_tokens[i].is_inside_string
        ):
        ...
        if self.all_tokens[i].spelling in white_listed_macros:
            if debug:
                print("Encountered white-listed macro", self.all_tokens[i].spelling)
        ...
        if (
            self.all_tokens[i].spelling == ":"
            and i + 1 < len(self.all_tokens)
            and self.all_tokens[i + 1].spelling == ":"
        ):
        ...
        while i < len(self.all_tokens):
            ...
            if self.all_tokens[i].spelling == ";" and len(bracket_stack) == 0:
                ...
                stack.pop()
                if debug:
                    self.print_entry("exitting process_simple_statement", i, stack)
                return i + 1
            elif self.all_tokens[i].spelling == ":":
                if self.all_tokens[i + 1].spelling != ":":
                    if debug:
                        print(
                            "found colon:",
                            i,
                            self.all_tokens[i].line_number,
                            self.all_tokens[i].is_inside_string,
                            self.all_tokens[i].start,
                            "n?s",
                            n_question_marks,
                            "i+1",
                            self.all_tokens[i + 1].spelling,
                        )
                    if n_question_marks == 0:
                        ...
                        if debug:
                            self.print_entry(
                                "label declaration; soldiering onwards", i, stack
                            )
                        return self.process_statement(i, stack)
                    else:
                        n_question_marks -= 1
                ...
                n_question_marks += 1
            elif self.all_tokens[i].spelling == open_scope and self.could_be_lambda(
                lambda_detection_stack[-1]
            ):
                if debug:
                    self.print_entry(
                        "found open scope inside simple statement", i, stack
                    )
                i = self.process_scope(i, stack)
                if debug:
                    self.print_entry(
                        "finished dealing with open scope inside simple statement",
                        i,
                        stack,
                    )
                del lambda_detection_stack[-1][
                    :
                ]  # empty out the list of the last entry in the lambda-detection stack
                continue
            elif self.all_tokens[i].spelling in open_brackets:
                bracket_stack.append(self.all_tokens[i].spelling)
                lambda_detection_stack.append([])
                if (
                    macro_without_ending_semicolon
                    and not encountered_first_lparen
                    and self.all_tokens[i].spelling == "("
                ):
                ...
                if len(bracket_stack) == 0:
                    ...
                    if (
                        self.var_in_class_dec_initializer(stack)
                        and self.all_tokens[i].spelling == "}"
                    ):
                        if debug:
                            self.print_entry(
                                "simple statement has turned out to be a variable!",
                                i,
                                stack,
                            )
                    ...
                    print(
                        "Closing bracket '"
                        + self.all_tokens[i].spelling
                        + "' found that does not match an opening bracket in",
                        self.filename,
                    )
                    sys.exit(1)
                if (
                    closing_bracket_for_opener[bracket_stack[-1]]
                    != self.all_tokens[i].spelling
                ):
                    print(
                        "Closing bracket does not match opening bracket in",
                        self.filename,
                    )
                ...
                if (
                    len(bracket_stack) == 0
                    and macro_without_ending_semicolon
                    and encountered_first_lparen
                ):
                    self.set_parent(i, stack)
                    stack.pop()
                    return i + 1
    ...
    def process_function_preamble_or_variable(self, i, stack):
        ...
        if debug:
            self.print_entry("process_function_preamble_or_variable", i, stack)
        classname = ""
        if stack[-1].type == "class-scope":
            classname = self.current_classname(i, stack)
            if debug:
                print("processing class constructor:", classname)
        ...
        while i < len(self.all_tokens):
            ...
            #    pass
                ...
                if funcname != "operator":
                    ...
                    if debug:
                        print(
                            "Read to end paren finished with token:",
                            self.all_tokens[i].spelling,
                        )
                    arglist = "ended"
                ...
                if debug:
                    self.print_entry(
                        "exitting process_function_preamble_or_variable", i, stack
                    )
                return i
                ...
                if len(bracket_stack) == 0:
                    print(
                        "Bracket stack is empty when encountering",
                        self.all_tokens[i].spelling,
                        "on line",
                        self.all_tokens[i].line_number + 1,
                    )
                ...
                if (
                    ...
                    print(
                        "Wrong closing brack encountered:",
                        self.all_tokens[i].spelling,
                        "on line",
                        self.all_tokens[i].line_number + 1,
                    )
            ...
            self.set_parent(i, stack)
            i += 1
        self.handle_read_all_tokens_error(
            "process_function_preamble_or_variable", stack
        )

    def process_for(self, i, stack):
        if debug:
            self.print_entry("process_for", i, stack)
        ...
        while i < len(self.all_tokens):
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if arglist == "not-yet-begun":
                    ...
                    stack.pop()
                    if debug:
                        self.print_entry("exitting process_for", i, stack)
                    return i
            self.set_parent(i, stack)
            i += 1
    ...
    def process_if(self, i, stack):
        ...
        if debug:
            self.print_entry("process_if", i, stack)
        ...
        while i < len(self.all_tokens):
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if self.all_tokens[j].spelling == "else":
                    ...
                    self.set_parent(
                        i, stack, "else"
                    )  # set the parent for the else as the if
                    stack.append(self.all_tokens[i])
                    i = self.process_statement(i + 1, stack)
                    stack.pop()  # remove else
                stack.pop()  # remove if
                if debug:
                    self.print_entry("exitting process_if", i, stack)
                return i
            self.set_parent(i, stack)
            i += 1
    ...
    def process_do_while(self, i, stack):
        ...
        if debug:
            self.print_entry("process_do_while", i, stack)
        ...
        while i < len(self.all_tokens):
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if debug:
                    self.print_entry("exitting process_do_while", i, stack)
                return i + 1
            self.set_parent(i, stack)
            i += 1
    ...
    def process_try(self, i, stack):
        ...
        if debug:
            self.print_entry("process_try", i, stack)
    ...
    def process_while(self, i, stack):
        ...
        if debug:
            self.print_entry("process_while", i, stack)
        ...
        while i < len(self.all_tokens):
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if debug:
                    self.print_entry("exitting process_while", i, stack)
                return i
            self.set_parent(i, stack)
            i += 1
    ...
    def process_namespace(self, i, stack):
        if debug:
            self.print_entry("process_namespace", i, stack)
            ...
            if self.all_tokens[i].spelling == "{":
                ...
                if debug:
                    if i < len(self.all_tokens):
                        ...
                        print("exiting process_namespace; all tokens processed")
                return i
            if self.all_tokens[i].spelling == ";":
                self.set_parent(i, stack)
                stack.pop()
    ...
    def process_class_decl(self, i, stack):
        assert (
            ...
            or self.all_tokens[i].spelling == "struct"
        )
        if debug:
            self.print_entry("process_class_decl", i, stack)
            ...
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if found_square_brackets:
                    ...
                    while i < len(self.all_tokens):
                        ...
                        if self.all_tokens[i].spelling == ";":
                            break
                            # case where struct is anon and struct variables declared next
                        i += 1
                    stack.pop()
                    if debug:
                        self.print_entry("exitting process_class_decl", i, stack)
                    return i + 1
            elif self.all_tokens[i].spelling == ":":
                if not seen_inheritance_colon:
                    if debug:
                        print(" " * len(stack), "encountered inheritance colon")
    ...
    def process_union(self, i, stack):
        ...
        if debug:
            self.print_entry("process_union", i, stack)
        ...
        while i < len(self.all_tokens):
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if debug:
                    self.print_entry("exitting process_union", i, stack)
                return i + 1
            self.set_parent(i, stack)
            i += 1
    ...
    def process_switch(self, i, stack):
        ...
        if debug:
            self.print_entry("process_switch", i, stack)
        ...
        while i < len(self.all_tokens):
            ...
            if self.all_tokens[i].spelling == "(":
                ...
                if debug:
                    self.print_entry("exitting process_switch", i, stack)
                return i
            self.set_parent(i, stack)
            i += 1
        self.handle_read_all_tokens_error("process_class_decl", stack)
    ...
    def process_case(self, i, stack):
        assert (
            self.all_tokens[i].spelling == "case"
            or self.all_tokens[i].spelling == "default"
        )
        if debug:
            self.print_entry("process_case", i, stack)
            ...
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                if self.all_tokens[i + 1].spelling != ":":
                    ...
                    while i < len(self.all_tokens):
                        ...
                        if (
                            i_spelling == "case"
                            or i_spelling == "default"
                            or i_spelling == "}"
                        ):
                            stack.pop()  # pop case-block
                            stack.pop()  # pop case
                            if debug:
                                self.print_entry("exitting process_case", i, stack)
                            return i
                        else:
                            i = self.process_statement(i, stack)
                else:
    ...
    def process_to_end_rcb(self, i, stack, scopename):
        if debug:
            self.print_entry("process_to_end_rcb", i, stack)
    ...
    def process_enum(self, i, stack):
        if debug:
            self.print_entry("process_enum", i, stack)
    ...
    def process_scope(self, i, stack):
        if debug:
            self.print_entry("process_scope", i, stack)
        ...
        while i < len(self.all_tokens):
            ...
            if self.all_tokens[i].spelling == "}":
                ...
                if debug:
                    self.print_entry("exitting process_scope", i, stack)
                return i + 1
            else:
    ...
    def process_privacy_declaration(self, i, stack):
        if debug:
            self.print_entry("process_privacy_declaration", i, stack)
        ...
        while i < len(self.all_tokens):
            if not self.all_tokens[i].is_visible or self.all_tokens[i].is_inside_string:
                ...
                stack.pop()
                if debug:
                    self.print_entry("exitting process_privacy_declaration", i, stack)
                return i + 1
            self.set_parent(i, stack)
            i += 1
    ...
    def next_visible_token_on_line(self, line_number, starting_token_ind=0):
        ...
        while i < len(self.line_tokens[line_number]):
            if self.line_tokens[line_number][i].is_visible:
                return i
        return i
    ...
    def determine_indentation_level(self, line_number):
        ...
        if first_token.spelling in self.macros_at_zero_indentation:
            ...
            if (
                first_token.spelling == "}"
                and first_token.is_visible
                and not first_token.is_inside_string
            ):
                self.line_indentations[line_number] = self.line_indentations[
                    first_token.parent.line_number
                ]
                ...
                self.line_indentations[line_number] = (
                    self.line_indentations[first_token.parent.line_number] + 1
                )
            ...
            # indent twice
            self.line_indentations[line_number] = (
                self.line_indentations[first_token.parent.line_number] + 2
            )
            ...
            if (
                ...
                self.line_indentations[line_number] = (
                    self.line_indentations[first_token.parent.parent.line_number] + 1
                )
            ...
            if (
                ...
                self.line_indentations[line_number] = (
                    self.line_indentations[first_token.parent.parent.parent.line_number]
                    + 1
                )
            else:
                self.line_indentations[line_number] = (
            ...
            if (
                self.line_tokens[line_number][0].spelling == "{"
                and first_token.is_visible
                and not first_token.is_inside_string
            ):
                self.line_indentations[line_number] = self.line_indentations[
            ...
            if (
                (first_token.spelling == "}" or first_token.type == "class-privacy")
                and first_token.is_visible
                and not first_token.is_inside_string
            ):
                self.line_indentations[line_number] = self.line_indentations[
            ...
            if (
                first_token.spelling == ")"
                and first_token is first_token.parent.children[-1]
            ):
            ...
            if first_token.parent.context() == "statement-scope":
                ...
                if (
                    self.line_tokens[first_token.parent.line_number][0]
                    is not first_token.parent
                ):
                    self.line_indentations[line_number] = (
                        self.line_indentations[first_token.parent.line_number] + 1
                    )
            ...
            # this statement has run on to a second line
                ...
                first_token.is_visible
                and first_token.spelling == ")"
                and first_token.parent.children[-2] is first_token
                and first_token.parent.children[-1].spelling == ";"
            ):
            ...
            if (
                first_token.is_visible
                and first_token.spelling == ">"
                and first_token.parent.children[-1] is first_token
            ):
    ...
    def line_from_line_tokens(self, line_number):
        ...
        if len(toks) == 0:
            ...
            for i, tok in enumerate(toks):
                ...
                if i + 1 != len(toks):
                    spellings.append(
                        " " * (toks[i + 1].start - tok.one_past_end)
                    )  # preserve spaces
            spellings.append("\n")
    ...
    def correct_spacing(self):
        for i, tok in enumerate(self.all_tokens):
            if not tok.is_visible or tok.is_inside_string:
                continue
            ...
            if tok.context() == "if":
                ...
                if tok.spelling == ";":
                    self.enforce_space_between(
                        tok, nexttok
                    )  # for ( int i = 1;_i<=..), for (..;_++i )
    ...
    def excise_token_and_move_upwards_and_after(
        self, moving_token, dest_tok, adjust_parentage=True, space_from_prev=1
    ):
    ...
    def delete_empty_line(self, old_line):
        ...
        for tok in self.all_tokens:
            if tok.line_number > old_line:
                tok.line_number -= 1
        self.line_tokens = (
            self.line_tokens[:old_line] + self.line_tokens[old_line + 1 :]
        )
    ...
    def insert_token_lines(self, tok_lines):
        ...
        self.line_tokens = (
            self.line_tokens[:firstline] + tok_lines + self.line_tokens[firstline:]
        )
    ...
    def token_is_scope_ender(self, tok):
        return (
            tok.is_visible
            and not tok.is_inside_string
            and tok.spelling == "}"
            and tok.parent.children[-1] is tok
            and tok.parent.type in self.scope_types
        )
    ...
    def adjust_parentage_of_comments_after(
        self, line, first_to_adjust, comments_new_parent, prev_child_of_comment
    ):
        # print("prev_child_of_comment:", "None" if not prev_child_of_comment else prev_child_of_comment.spelling)
        new_child_ind = (
            comments_new_parent.children.index(prev_child_of_comment) + 1
            if prev_child_of_comment
            else 0
        )
    ...
    def get_parent_for_eol_comments_for_else(self, else_tok, last_desc, line_number):
        ...
        if line_number == last_desc.line_number:
            ...
            prev_child = (
                iftok
            )  # comments will be appended after iftok in the same context as iftok
        else:
    ...
    def move_line_upwards_and_after(
        self, tok, tok_before, last_visible_ind, comments_new_parent, prev_child
    ):
        ...
        while i < len(orig_line):
            ...
            if itok.is_visible or i <= last_visible_ind:
                self.excise_token_and_move_upwards_and_after(
                    itok,
                    before_tok,
                    False,
                    1 if i == tok_line_index else itok.start - prev_end,
                )
                before_tok = itok
            else:
                ...
                self.excise_token_and_move_upwards_and_after(
                    ...
                    last_tok,
                    False,
                    1 if i + 1 == last_visible_ind else itok.start - prev_end,
                )
                # itok.parent.children.remove(itok)
            ...
            prev_end = itok_end
            i += 1
        self.adjust_parentage_of_comments_after(
            orig_line, last_visible_ind + 1, comments_new_parent, prev_child
        )
        ...
        for line_number in range(tok_before.line_number + 1, orig_line_number + 1):
            for tok in self.line_tokens[line_number]:
                ...
                assert (
                    not tok.is_visible
                    and not tok.invisible_by_macro
                    and not tok.is_preprocessor_directive
                )
            new_child_ind_plus_one = self.adjust_parentage_of_comments_after(
                self.line_tokens[line_number], 0, comments_new_parent, prev_child
            )
    ...
    def find_rcb_child(self, tok):
        ...
        rcbs = [
            ...
            if x.is_visible and not x.is_inside_string and x.spelling == "}"
        ]
        assert len(rcbs) == 1
        return rcbs[0]
    ...
    def intervening_macro_tokens(self, tok1, tok2):
        ...
        for line_number in range(line1, line2 + 1):
            ...
            while ind < len(toks):
                ...
                if tok is tok2:
                    break
                if tok.is_preprocessor_directive or tok.invisible_by_macro:
                    return True
                ind += 1
        return False
    ...
    def adjust_else(self, tok):
        ...
        if prev_child.type == "if-scope":
            ...
            if (
                if_rcb.line_number != tok.line_number
                and if_lcb.line_number != if_rcb.line_number
                and not self.intervening_macro_tokens(if_rcb, tok)
            ):
                # ok -- move the "else" up to behind the rcb
                last_visible_ind, comments_new_parent, prev_child = self.get_parent_for_eol_comments_for_else(
                    tok, last_desc, tok.line_number
                )
                ...
                self.move_line_upwards_and_after(
                    tok, if_rcb, last_visible_ind, comments_new_parent, prev_child
                )
        ...
        if else_body.type == "if":
            if (
                tok.line_number != else_body.line_number
                and not self.intervening_macro_tokens(tok, else_body)
            ):
                # ok, make sure that the whole line that the "if" statement is on ends up on this line
                last_visible_ind, comments_new_parent, prev_child = self.get_parent_for_eol_comments_for_else(
                    tok, last_desc, else_body.line_number
                )
                ...
                self.move_line_upwards_and_after(
                    else_body, tok, last_visible_ind, comments_new_parent, prev_child
                )
            ...
            if (
                tok.line_number != else_lcb.line_number
                and not self.intervening_macro_tokens(tok, else_body)
            ):
                ...
                if len(self.line_tokens[orig_line_number]) == 0:
            ...
            if len(else_lcb.children) != 1:
                ...
                if (
                    last_before_rcb.line_number == else_rcb.line_number
                    and else_rcb.line_number != else_body.line_number
                ):
            ...
            if (
                last_es_desc.line_number != tok.line_number
                and not self.intervening_macro_tokens(tok, else_body)
            ):
                # print('we need to add a pair of curly braces')
                lcb_tok = self.add_left_curly_brace_after(tok)
                lcb_tok.type = "else-scope"
                self.adjust_token_tree(
                    lcb_tok, tok.children[0], else_body
                )  # start with the first child of the else (possibly a comment)
                ...
                if self.visible_tokens_on_line_after(last_es_desc):
                    print(
                        "Possible bug identified on line",
                        last_es_desc.orig_line_number + 1,
                        "of file",
                        self.filename,
                        "1",
                    )
                ...
                if self.line_tokens[last_es_desc.line_number][-1] is not last_es_desc:
                    if self.visible_tokens_on_line_after(last_es_desc):
                        next_visible = self.next_visible_token_on_line_after(
                            last_es_desc
                        )
    ...
    def adjust_if(self, tok):
        ...
        if if_body.type == "if-scope":
            ...
            if (
                end_paren.line_number != if_body.line_number
                and not self.intervening_macro_tokens(end_paren, if_body)
            ):
                ...
                if len(if_body.children) == 1:
                    ...
                    if lcb.line_number != rcb.line_number:
                        ...
                        rcb.start = lcb.one_past_end
                        rcb.one_past_end = rcb.start + 1
                        self.delete_empty_lines_between(
                            lcb.line_number + 1, orig_rcb_line_number
                        )
                    # now move the lcb and rcb up behind the end_paren
                    last_visible_ind = self.find_last_visible_token_on_line(
                        rcb.line_number
                    )
                    ...
                    self.delete_empty_lines_between(
                        end_paren.line_number + 1, orig_lcb_line_number
                    )
            ...
            if (
                if_body.line_number != right_curly_brace.line_number
                and len(if_body.children) != 1
            ):
                last_before_rcb = self.last_descendent(if_body.children[-2])
                if last_before_rcb.line_number == right_curly_brace.line_number:
                    # print('ok! move rcb to its own line')
                    self.excise_right_curly_brace_and_move_to_next_line_on_own(
                        right_curly_brace
                    )
            elif len(if_body.children) == 1:
                # make sure the {}s are on the same line together
                if (
                    right_curly_brace.line_number != if_body.line_number
                    and not self.intervening_macro_tokens(if_body, right_curly_brace)
                ):
                    self.put_empty_curly_braces_on_line_together(
                        if_body, right_curly_brace
                    )
        else:
            ...
            if (
                last_is_desc.line_number != last_ic_desc.line_number
                and not self.intervening_macro_tokens(tok, if_body)
            ):
                # print('we need to add a pair of curly braces')
                lcb_tok = self.add_left_curly_brace_after(end_paren)
                lcb_tok.type = "if-scope"
                self.adjust_token_tree(
                    lcb_tok, tok.children[tok.children.index(if_cond) + 1], if_body
                )
                ...
                if self.visible_tokens_on_line_after(last_is_desc):
                    print(
                        "Possible bug identified on line",
                        last_is_desc.orig_line_number + 1,
                        "of file",
                        self.filename,
                        "3",
                    )
                ...
                if self.line_tokens[last_is_desc.line_number][-1] is not last_is_desc:
                    if self.visible_tokens_on_line_after(last_is_desc):
                        ...
                        if (
                            not self.token_is_scope_ender(next_visible)
                            and next_visible.type != "else"
                        ):
                            print(
                                "Possible bug identified on line",
                                last_is_desc.orig_line_number + 1,
    ...
    def adjust_for(self, tok):
        ...
        if for_body.type == "for-scope":
            ...
            if len(for_body.children) != 1:
                ...
                if (
                    last_before_rcb.line_number == right_curly_brace.line_number
                    and right_curly_brace.line_number != for_body.line_number
                ):
                    # print('ok! move rcb to its own line')
                    self.excise_right_curly_brace_and_move_to_next_line_on_own(
                        right_curly_brace
                ...
                if (
                    ...
                    and not self.intervening_macro_tokens(for_body, rcb)
                ):
            ...
            if (
                last_fs_desc.line_number != last_fd_desc.line_number
                and not self.intervening_macro_tokens(tok, for_body)
            ):
                # print('we need to add a pair of curly braces')
                lcb_tok = self.add_left_curly_brace_after(end_paren)
                lcb_tok.type = "for-scope"
                self.adjust_token_tree(
                    lcb_tok, tok.children[tok.children.index(for_dec_tok) + 1], for_body
                )
                ...
                if self.visible_tokens_on_line_after(last_fs_desc):
                    print(
                        "Possible bug identified on line",
                        last_fs_desc.orig_line_number + 1,
                        "of file",
                        self.filename,
                        "5",
                    )
    ...
    def equiv_if_both_empty_or_if_neither_empty_and_equiv(
        self, other, i_this, i_other, stack
    ):
        if debug_equiv:
            self.enter_equiv(
                other,
                "equiv_if_both_empty_or_if_neither_empty_and_equiv",
                i_this,
                i_other,
                stack,
            )
        ...
        if tok_this.type == "empty-statement" or tok_other == "empty-statement":
            if tok_this.equivalent(tok_other):
                ...
                tok_this.type == "empty-statement"
                and tok_other.type in self.scope_types
            ):
                return self.equiv_to_scoped_statement(
                    other, i_this, i_other, stack, self.empty_statement_equiv
                )
            elif tok_this.type in self.scope_types:
                retval = other.equiv_to_scoped_statement(
                    self, i_other, i_this, stack, other.empty_statement_equiv
                )
    ...
    def equiv_to_scoped_statement(self, other, i_this, i_other, stack, func):
        ...
        # print(func)
        still_good, i_this, i_other = func(
    ...
    def for_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "for_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        assert tok_this.type == "for"
        if tok_other.type == "empty-statement":
            return self.for_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.for_equiv
            )
        ...
        i_this += 1
        i_other += 1
        still_good, i_this, i_other = self.paren_block_equiv(
            other, i_this, i_other, "for", "for-declaration", stack
        )
        if not still_good:
            stack.pop("paren block not equiv")
            return still_good, i_this, i_other
        retval = self.equiv_if_both_empty_or_if_neither_empty_and_equiv(
            other, i_this, i_other, stack
        )
        stack.pop()
        return retval
    ...
    def if_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "if_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        assert tok_this.type == "if"
        if tok_other.type == "empty-statement":
            return self.if_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.if_equiv
            )
        if tok_other.type != "if":
            return False, i_this, i_other

        stack.append("if_equiv")
        still_good, i_this, i_other = self.paren_block_equiv(
            other, i_this, i_other, "if", "if-condition", stack
        )
        if not still_good:
            stack.pop()
            return still_good, i_this, i_other
        i_this, i_other = self.two_next_visible(other, i_this, i_other)
        still_good, i_this, i_other = self.equiv_if_both_empty_or_if_neither_empty_and_equiv(
            other, i_this, i_other, stack
        )
        if not still_good:
            stack.pop("statements not equiv")
            return still_good, i_this, i_other
        ...
        if tok_this.type == "else" or tok_other.type == "else":
            if tok_this.type != tok_other.type:
                stack.pop("type mismatch")
                return False, i_this2, i_other2
    ...
    def paren_block_equiv(
        self, other, i_this, i_other, end_context, left_paren_type, stack
    ):
        if debug_equiv:
            self.enter_equiv(other, "paren_block_equiv", i_this, i_other, stack)
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            tok_other = other.all_tokens[i_other]
            if (
                tok_this.context() == end_context
                and tok_this.type != left_paren_type
                and found_open_paren
            ):
                stack.pop()
                if debug_equiv:
                    print(
                        " " * len(stack) + "leaving paren_block_equiv", i_this, i_other
                    )
                return True, i_this, i_other
        ...
        print(
            "Ran out of tokens in paren_block_equiv, end_context =",
            end_context,
            "left_paren_type =",
            left_paren_type,
        )
    ...
    def do_while_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "do_while_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        assert tok_this.type == "do-while"
        if tok_other.type == "empty-statement":
            return self.do_while_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.do_while_equiv
            )
        if tok_other.type != "do-while":
            return False, i_this, i_other
        ...
        still_good, i_this, i_other = self.paren_block_equiv(
            other, i_this, i_other, "do-while", "do-while-condition", stack
        )
        if not still_good:
            stack.pop()
            return still_good, i_this, i_other
    ...
    def while_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "while_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        assert tok_this.type == "while"
        if tok_other.type == "empty-statement":
            return self.while_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )

        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.while_equiv
            )
        if tok_other.type != "while":
            return False, i_this, i_other
        stack.append("while_equiv")
        i_this, i_other = self.two_next_visible(other, i_this + 1, i_other + 1)
        still_good, i_this, i_other = self.paren_block_equiv(
            other, i_this, i_other, "while", "while-condition", stack
        )
        if not still_good:
            stack.pop()
            return still_good, i_this, i_other
    ...
    def namespace_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "namespace_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.namespace_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        ...
        while i_this < len(self.all_tokens) and i_other < len(self.all_tokens):
            ...
            if tok_this2.parent is not tok_this or tok_other2.parent is not tok_other:
                if debug_equiv:
                    print(
                        " " * len(stack) + "leaving snamespace_equiv",
                        tok_this2.spelling,
    ...
    def class_equiv(self, other, i_this, i_other, stack):
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            if tok_this.is_inside_string or tok_other.is_inside_string:
                ...
                if not tok_this.equivalent(tok_other):
                    ...
                    still_good, i_this, i_other = self.statement_equiv(
                        other, i_this, i_other, stack
                    )
                    if not still_good:
                        stack.pop()
        ...
        stack.pop()
        return (
            i_this == len(self.all_tokens) and i_other == len(other.all_tokens),
            i_this,
            i_other,
        )
    ...
    def class_privacy_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "class_privacy_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.class_privacy_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
    ...
    def union_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "union_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.union_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
    ...
    def switch_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "switch_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.switch_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.switch_equiv
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
    ...
    def switch_scope_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "switch_scope_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            if tok_this.type == "case" or tok_other.type == "case":
                if tok_this.type != tok_other.type:
                    stack.pop("cases not equal")
                    return False, i_this, i_other
                still_good, i_this, i_other = self.case_equiv(
                    other, i_this, i_other, stack
                )
                if not still_good:
                    stack.pop("case not equiv")
                    return still_good, i_this, i_other
                continue
            if (tok_this.parents_last_child() and tok_this.regular_rcb()) or (
                tok_other.parents_last_child() and tok_other.regular_rcb()
            ):
    ...
    def case_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "case_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.case_equiv
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            if tok_this.context() == "case" or tok_other.context() == "case":
                ...
                tok_this.type == "empty-statement"
                or tok_other.type == "empty-statement"
            ):
                if tok_this.type == "empty-statement":
                    i_this += 1
                if tok_other.type == "empty-statement":
                    i_other += 1
                i_this, i_other = self.two_next_visible(other, i_this, i_other)
                ...
                tok_this.context() == "switch-scope" and tok_this.parents_last_child()
            ) or (
    ...
    def scope_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "scope_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.scope_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        ...
        if tok_other.type not in self.scope_types:
            ...
            still_good, i_this, i_other = self.statement_equiv(
                other, self.find_next_visible_token(i_this + 1), i_other, stack
            )
            if not still_good:
                return still_good, i_this, i_other
            i_this = self.find_next_visible_token(i_this)
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            if debug_equiv:
                print(
                    " " * len(stack) + "scope equiv while loop",
                    tok_this.spelling,
                    tok_other.spelling,
                )
            if (
                ...
                if debug_equiv:
                    print(" " * len(stack) + "leaving scope_equiv", i_this, i_other)
                stack.pop()
                if tok_this.spelling != tok_other.spelling:
                    return False, i_this, i_other
                return True, i_this + 1, i_other + 1
                ...
                if debug_equiv:
                    print(
                        " " * len(stack)
                        + "scope equiv returned from self.statement_equiv",
                        i_this,
                        i_other,
                    )
                if not still_good:
    ...
    def empty_statement_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "empty_statement_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        ...
        if tok_other.type == "empty-statement":
            ...
            return self.statement_equiv(
                other, self.find_next_visible_token(i_this + 1), i_other, stack
            )

    def simple_statement_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "simple_statement_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.simple_statement_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )

        if tok_other.type in self.scope_types:
            return self.equiv_to_scoped_statement(
                other, i_this, i_other, stack, self.simple_statement_equiv
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        i_this, i_other = self.two_next_visible(other, i_this + 1, i_other + 1)
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            if debug_equiv:
                print(
                    " " * len(stack) + "simple statement equiv while loop",
                    tok_this2.spelling,
                    tok_other2.spelling,
                )
            if tok_this2.parent is not tok_this or tok_other2.parent is not tok_other:
                if debug_equiv:
                    print(
                        " " * len(stack) + "leaving simple_statement_equiv",
                        tok_this2.spelling,
                        tok_this2.parent.spelling,
                        tok_other2.spelling,
                        tok_other2.parent.spelling,
    ...
    def substatement_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "substatement_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            if tok_this2.parent is not tok_this or tok_other2.parent is not tok_other:
                ...
                if debug_equiv:
                    print(
                        " " * len(stack) + "leaving substatement_equiv",
                        tok_this2.spelling,
                        tok_this2.parent.spelling,
                        tok_other2.spelling,
                        tok_other2.parent.spelling,
    ...
    def template_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "template_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.function_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            if debug_equiv:
                print("template_equiv while loop:", i_this, i_other)
            tok_this, tok_other = self.two_toks(other, i_this, i_other)
            if tok_this.spelling == ">" or tok_other.spelling == ">":
                if tok_this.spelling == ">" and tok_this.parents_last_child():
                    if (
                        tok_this.equivalent(tok_other)
                        and tok_other.parents_last_child()
                    ):
                        return True, i_this + 1, i_other + 1
                    return False, i_this, i_other
    ...
    def function_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "function_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.function_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            tok_this, tok_other = self.two_toks(other, i_this, i_other)
            if (
                tok_this.type == "function-scope"
                or tok_other.spelling == "function-scope"
            ):
                retval = self.statement_equiv(other, i_this, i_other, stack)
                stack.pop()
                return retval
    ...
    def try_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "try_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.try_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            ...
            i_this, i_other = self.two_next_visible(other, i_this + 1, i_other + 1)
            still_good, i_this, i_other = self.paren_block_equiv(
                other, i_this, i_other, "catch", "catch-arg", stack
            )
            if not still_good:
                stack.pop()
                return still_good, i_this, i_other
    ...
    def enum_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "enum_equiv", i_this, i_other, stack)
        tok_this, tok_other = self.two_toks(other, i_this, i_other)
        if tok_other.type == "empty-statement":
            return self.enum_equiv(
                other, i_this, other.find_next_visible_token(i_other + 1), stack
            )
        if not tok_this.equivalent(tok_other):
            return False, i_this, i_other
        ...
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            tok_this, tok_other = self.two_toks(other, i_this, i_other)
            if tok_this.context() != tok_other.context():
                return False, i_this, i_other
            if tok_this.context() != "enum" and tok_this.context() != "enum-scope":
                return True, i_this, i_other
            if not tok_this.equivalent(tok_other):
                return False, i_this, i_other
            i_this, i_other = self.two_next_visible(other, i_this + 1, i_other + 1)
    ...
    def statement_equiv(self, other, i_this, i_other, stack):
        if debug_equiv:
            self.enter_equiv(other, "statement_equiv", i_this, i_other, stack)
    ...
    def enter_equiv(self, other, fname, i_this, i_other, stack):
        ...
        print(
            "%s %d "
            % (
                self.all_tokens[i_this].spelling,
                self.all_tokens[i_this].line_number + 1,
            )
        ...
        print(
            "%s %d "
            % (
                other.all_tokens[i_other].spelling,
                other.all_tokens[i_other].line_number + 1,
            )
    ...
    def equivalent(self, other):
        ...
        i_this, i_other = self.two_next_visible(other, i_this, i_other)
        while i_this < len(self.all_tokens) and i_other < len(other.all_tokens):
            still_equiv, i_this, i_other = self.statement_equiv(
                other, i_this, i_other, stack
            )
            if not still_equiv:
                return False, i_this, i_other
        ...
        if i_this < len(self.all_tokens):
            ...
            if i_this < len(self.all_tokens):
                print(
                    "unprocessed tokens in self for",
                    self.filename,
                    ":",
                    i_this + 1,
                    "of",
...
def beautify_file(filename, overwrite, opts=None):
    ...
    if any([line.endswith("\r\n") for line in orig_lines]):
        print(
            "WARNING: file %s contains DOS-style line endings. This may cause the beautifier to fail."
            % filename
        )
    for macro_set in macro_sets:
        ...
        if opts:
            if (
                opts.pound_if_setting == "take_if"
                or opts.pound_if_setting == "take_else"
            ):
    ...
    if overwrite or debug:
        ...
        for macro_set in macro_sets:
            ...
            if not equiv:
                ...
                print(
                    "MACRO set: "
                    + ", ".join(macro_set)
                    + " did not produce the same tree in the original and beautified code for",
                    filename,
                )
        ...
        if overwrite and all_good:
            ...
            print(
                "Did not beautify",
                filename,
                "because tree differed in the presence of some macros",
            )
...
class SmartStack:
    ...
    def append(self, str):
        if self.debug:
            print(" " * len(self.stack) + "appending", str)
        self.stack.append(str)

    def pop(self, msg=""):
        str = self.stack.pop()
        if self.debug:
            print(" " * len(self.stack) + "popped", str, msg)
