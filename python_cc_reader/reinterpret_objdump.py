...
def relabel_sections( alllines ) :
   ...
   for line in goodlines :
      ...
      if re_half_sect_header.match( line ) : # no processing needs to be done for this line
         newlines.append( line )
         currsect = line.split("<")[1].split(">")[0]
         continue
      newline = orig_labels[ currsect ][ line.split(":")[ 0 ].strip() ] + ":" + line.split(":")[ 1 ]
      for label in regexes :
         if regexes[ label ].search( newline ) :
            ...
            if toks[ 0 ].strip() in orig_labels[ refsect ] :
               #print newline,
               newline = newline_split[ 0 ] + label + " "*(7-len(label)) + orig_labels[ refsect ][ toks[ 0 ].strip() ] + " " + "".join( toks[ 1: ] )  + "\n"
               #print newline,
            else :
               #print toks[ 0 ], "on line", count, "not found in orig_lables dictionary for reference section", refsect
               newline = newline_split[ 0 ] + label + " "*(7-len(label)) + "funcend" + " " + "".join( toks[ 1: ] )  + "\\n"
            break
