...
class SingleSimulation(BaseMinFunc):
    ...
    def __init__(self, data_folder, orig_weight=np.ones(N_SCORE_TERMS),
        ...
        self.curr_weight = np.array(orig_weight)
	self._orig_weight = np.array(orig_weight)
        ...
        if down_sampling_ratio is not None:
            ...
            del resampled_data
            del data
        self.histograms = [self._energy2hist(data[:, 1], data[:, 0])
                      for data in raw_data]

        dos_raw = np.sum(self.histograms, axis=0) / normalization
	...
	torsion_volume = 1.
        ...
        # For legacy data
        # self._score_terms = full_data[:, 2:] / orig_weight
    ...
    def get_ST_weights(self, kt_list):
        ...
        for kt1, kt2 in zip(kt_list[:-1], kt_list[1:]):
            hist1 = np.column_stack((WHAM_ENERGIES, dos * np.exp(-WHAM_ENERGIES / kt1)))
            hist2 = np.column_stack((WHAM_ENERGIES, dos * np.exp(-WHAM_ENERGIES / kt2)))
	...
	### for Kalli, I don't think I need this, set to zero
        #score_weight_diff = self.curr_weight - self._orig_weight
	#score_weight_diff = 0
        #energy = self._energy0 + self._score_terms.dot(score_weight_diff)
	energy = self._energy0
        # energy = self._score_terms.dot(self.curr_weight)
        return energy
...
class SingleHistSimulation(object):
    ...
    def __init__(self, data_folder, torsion_volume):
        ...
        for kt, file_list in kt_to_fn.iteritems():
            raw_data.append(
                np.vstack(util.load_2d_bin_gz(fn) for fn in file_list))
            self.kt_list.append(kt)

        self.histograms = [_energy2hist(data[:, 1], data[:, 0])
                          for data in raw_data]

        # Scores for Hist bins. Assumes all the hist_scores.gz files are the same
        #self.hist_scores = util.load_1d_bin_gz(
        #    _get_file_names(data_folder, '_hist_scores.gz')[0], dtype=np.float64)
        ...
        #    self.hist_list.append(hist_counts)
        #    self.kt_list.append(kt)

        #_, dos_raw = _wham(self.hist_list, self.kt_list, self.hist_scores)
        _, dos_raw = _wham(self.histograms, self.kt_list)
        #seq = self.name.split('_')
        #torsion_volume = util.torsion_volume(*seq)
	#torsion_volume = 1.
        normalization = sum(dos_raw) / torsion_volume
        self._dos = dos_raw / normalization
        #self._dos_scores = self.hist_scores
	self.hist_scores = WHAM_ENERGIES
	self._dos_scores = WHAM_ENERGIES
    ...
    def get_dos(self):
        return self._dos
    def get_hist_list(self):
        return self.hist_list
    def get_hist_scores(self):
        return self.hist_scores
...
class TurnerRuleMinimizeFunc(BaseMinFunc):
    ...
    def __init__(self, single_simulations, dangling_weight=0,
        ...
        def update_simulation_and_target_value(seq, is_duplex=True):
            self._simulation_dict[seq] = get_simulation_from_seq(seq, single_simulations, symmetrize=False)
            self._target_values[seq] = value
            if is_duplex and seq[0][-1] in 'au':
                self._target_values[seq] += util.DG_TERMINAL[('a', 'u')]
            self._target_values[seq] /= KT_IN_KCAL

        for seq, value in util.DG_CANONICAL.iteritems():
