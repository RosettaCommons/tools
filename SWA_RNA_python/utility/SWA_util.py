...
def	popen_and_readlines(command, Is_master=False, tag=""):
	...
	if(exists( temp_data_filename)):
		sleep(5)
		if(exists( temp_data_filename)): error_exit_with_message("temp_data_filename %s already exist!" %(temp_data_filename) )
	...
	if(exists( temp_data_filename)==False): error_exit_with_message("temp_data_filename %s doesn't not exist!" %(temp_data_filename) )
